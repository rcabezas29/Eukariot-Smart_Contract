...
contract OurEventBadgee is ERC721{
    ...
    function mintBadge(string memory _name, string memory _eventType, address _account) public {
        require(owner == msg.sender);
        uint256 badgeId = eventBadges.length;
        eventBadges.push(EventBadge(_name, block.timestamp, _eventType));
        _mint(_account, badgeId);
    }
    
    function totalSupply() public view returns (uint256) {
        return eventBadges.length;
    }
}
